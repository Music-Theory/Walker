namespace Walker.Data.Vector.Generic.Plane {
	...
	/// 2-dimensional vectors. Taken from a branch of SFML.Net,
	/// because they don't have a .NET Core version.
	/// </summary>
	...
	public struct Vector2<T> : IEquatable<Vector2<T>> {
		...
		public static Vector2<T> operator -(Vector2<T> v1, Vector2<T> v2) {
			return new Vector2<T>(Operator<T>.Sub(v1.X, v2.X), Operator<T>.Sub(v1.Y, v2.Y));
		}
		...
		public static Vector2<T> operator *(Vector2<T> v, T x) {
			return new Vector2<T>(Operator<T>.Mult(v.X, x), Operator<T>.Mult(v.Y, x));
		}
		...
		public static Vector2<T> operator *(T x, Vector2<T> v) {
			return new Vector2<T>(Operator<T>.Mult(x, v.X), Operator<T>.Mult(x, v.Y));
		}
		...
		public static Vector2<T> operator *(Vector2<T> x, Vector2<T> y) {
			return new Vector2<T>(Operator<T>.Mult(x.X, y.X), Operator<T>.Mult(x.Y, y.Y));
		}
		...
		/// <returns>v / x</returns>
		public static Vector2<T> operator /(Vector2<T> v, T x) {
			return new Vector2<T>(Operator<T>.Div(v.X, x), Operator<T>.Div(v.Y, x));
		}
		...
		/// <returns>x / y</returns>
		public static Vector2<T> operator /(Vector2<T> x, Vector2<T> y) {
			return new Vector2<T>(Operator<T>.Div(x.X, y.X), Operator<T>.Div(x.Y, y.Y));
		}
